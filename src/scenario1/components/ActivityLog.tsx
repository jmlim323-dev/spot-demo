...
export default function ActivityLog({ entries, agentEntries }: Props) {
  return (
    <div style={{ borderTop: "none", flex: 1, display: "flex", minHeight: 0 }}>

      {/* 좌: 활동 로그 */}
      <div style={{ flex: 1, borderRight: "1px solid #F3F4F6", display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
        <div style={{ padding: "7px 12px", borderBottom: "1px solid #F3F4F6", background: "#FAFAFA", flexShrink: 0 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: "#374151" }}>📋 순찰 활동 로그</span>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "8px 12px" }}>
          {[...entries].reverse().map((e, i) => (
            <div key={i} style={activityLog.row}>
              <span style={activityLog.time}>{e.time}</span>
              <span style={activityLog.msgColor(e.type)}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
        <div style={{ padding: "7px 12px", borderBottom: "1px solid #F3F4F6", background: "#FAFAFA", flexShrink: 0 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: "#5B21B6" }}>🤖 AI 자동 조치 이력</span>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "8px 12px" }}>
